<template>
  <div
    :class="[
      ...
      'dark:bg-zinc-800 bg-zinc-100 dark:border-neutral-700/60 border-neutral-300/70',
  ...
  >
    <div class="flex justify-between items-start gap-3">
      ...
      <span class="text-zinc-500 dark:text-zinc-400 text-xs shrink-0">{{ postedAt }}</span>
    </div>

    <h3 class="mt-3 font-semibold text-base leading-snug">{{ title }}</h3>
    <p v-if="body" class="mt-1.5 text-zinc-500 dark:text-zinc-400 text-sm line-clamp-4">
      {{ body }}
    </p>
    ...
    <div class="flex justify-between items-center mt-4">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <span class="text-zinc-500 dark:text-zinc-400 text-xs truncate">{{ author.name }}</span>
      </div>
...
const categoryStyle = computed(() => {
  const map = {
    ...
    neutral: 'dark:bg-zinc-700 bg-neutral-100 dark:text-zinc-300 text-zinc-700',
  }
