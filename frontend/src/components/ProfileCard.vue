<template>
  <div :class="[
    ...
    <p v-if="bio" class="mt-2 text-zinc-500 dark:text-zinc-400 text-sm line-clamp-3">
      {{ bio }}
    </p>
    ...
    <div v-if="stats && stats.length" class="flex justify-center items-center gap-6 mt-4 w-full">
      <div v-for="stat in stats" :key="stat.label" class="text-center">
        ...
        <p class="text-zinc-500 dark:text-zinc-400 text-xs">{{ stat.label }}</p>
      </div>
    </div>
...
const statusColor = computed(() => {
  const map = {
    ...
    away: 'bg-gold',
    offline: 'dark:bg-zinc-600 bg-zinc-400',
  }
  return map[props.status] ?? 'dark:bg-zinc-600 bg-zinc-400'
})
</script>
