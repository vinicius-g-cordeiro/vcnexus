<template>
  ...
  >
    ...
    <span v-else-if="$slots.icon || icon" class="shrink-0 [&>svg]:h-4 [&>svg]:w-4">
      <slot name="icon">
        <span v-html="icon" />
      </slot>
    </span>
    ...
    <span
      v-if="!loading && ($slots['icon-right'] || iconRight)"
      class="shrink-0 [&>svg]:h-4 [&>svg]:w-4"
    >
...
<script setup>
/**
 * AppButton.vue — single configurable button for the whole system.
 ...
 * <AppButton variant="outline" to="/users">Back to users</AppButton>
 * <AppButton variant="danger" modal="confirm-delete" @open-modal="openModal">Delete</AppButton>
 * <AppButton native-type="submit" :loading="isSaving">Create account</AppButton>
 * <AppButton variant="ghost" href="https://docs.example.com" target="_blank">Docs</AppButton>
 */
...
const variantClass = computed(() => {
  const map = {
    ...
    secondary:
      'dark:bg-neutral-700 bg-neutral-300 dark:text-neutral-50 text-neutral-900 hover:dark:bg-neutral-600 hover:bg-neutral-400 focus:ring-emerald-500',
    outline:
      'border dark:border-neutral-600 border-neutral-400 dark:text-neutral-50 text-neutral-900 hover:bg-neutral-300 dark:hover:bg-neutral-700 focus:ring-emerald-500',
    ghost:
      'dark:text-neutral-50 text-neutral-900 hover:bg-neutral-300 dark:hover:bg-neutral-700 focus:ring-emerald-500',
    danger:
