...
export function initDarkMode() {
  ...
  const prefersDark = window.matchMedia("(prefers-color-scheme: app-dark)").matches;

  isDark.value = stored ? stored === "app-dark" : prefersDark;
...
export function useDarkMode() {
  function setDark(value) {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(STORAGE_KEY, value ? "app-dark" : "app-light");
    }
  }
