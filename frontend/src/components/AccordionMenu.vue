  ...
  <ul class="m-0 p-0 w-full list-none" :class="{ 'pl-2': depth > 0 }">
    <li v-for="(item, index) in items" :key="item.href ?? item.label ?? index" class="w-full">
      ...
      <RouterLink v-if="!hasChildren(item) && item.href" :to="item.href" class="flex items-center hover:bg-emerald-500 px-3 py-2.5 w-full text-zinc-900 dark:text-zinc-200 text-sm transition-colors" active-class="border-b border-b-emerald-500 text-emerald-600 font-semibold hover:bg-emerald-50" @click="$emit('navigate', item)"
        ...
        <span class="flex-1 text-left truncate" v-show="!collapsed">{{ item.label }}</span>
      </RouterLink>
      ...
      <template v-else-if="hasChildren(item) && !item.href">
        ...
        <transition enter-active-class="transition-[grid-template-rows] duration-200 ease-out" leave-active-class="transition-[grid-template-rows] duration-200 ease-in" enter-from-class="grid-rows-[0fr]" enter-to-class="grid-rows-[1fr]" leave-from-class="grid-rows-[1fr]"
          ...
          <div v-if="isOpen(index)" class="grid overflow-hidden">
            <div class="min-h-0">
              <AccordionMenu :collapsed="collapsed" :items="item.children" :depth="depth + 1" @navigate="$emit('navigate', $event)" />
            </div>
          </div>
        </transition>
      </template>
