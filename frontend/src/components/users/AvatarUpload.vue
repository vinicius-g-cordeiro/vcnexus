  ...
  <Fieldset legend="Avatar" description="This is displayed on your profile and across the system.">
    <div class="flex items-center gap-6">
      <div class="relative shrink-0">
        <img v-if="previewUrl" :src="previewUrl" :alt="name" class="rounded-full ring-2 ring-emerald-500 w-20 h-20 object-cover" />
        ...
        <button v-if="previewUrl" type="button" class="-top-1 -right-1 absolute bg-red-500 hover:bg-red-600 p-1 rounded-full text-white transition-colors" aria-label="Remove avatar" @click="removeAvatar">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2">
          ...
          <Button v-if="previewUrl" size="sm" variant="ghost" @click="removeAvatar">
            Remove
          </Button>
        </div>
      ...
      <input ref="fileInput" type="file" accept="image/png, image/jpeg, image/gif" class="hidden" @change="handleFileChange" />
    </div>
  </Fieldset>
</template>
