  ...
  <header :class="[
    ...
    <div class="flex items-center gap-3 mx-auto px-4 max-w-7xl h-16">
      ...
      <nav class="hidden lg:flex items-center gap-1 ml-6">
        <a v-for="item in navItems.filter((i) => !i.children)" :key="item.label" :href="item.href" class="justify-between hover:bg-neutral-300 dark:hover:bg-neutral-700 px-3 py-2 rounded-md font-medium text-sm align-middle transition-colors">
        ...
        <Dropdown v-for="item in navItems.filter((i) => i.children)" :key="item.label">
          <template #trigger="{ toggle }">
            <button type="button" class="inline-flex items-center gap-1 hover:bg-neutral-300 dark:hover:bg-neutral-700 px-3 py-2 rounded-md font-medium text-sm transition-colors" @click="toggle">
              <slot name="icon" v-if="item.icon">
                <i :class="item.icon"></i>
              </slot>
              {{ item.label }}
      ...
      <Dropdown align="right" v-if="authStore.sessionUser">
        <template #trigger="{ toggle }">
          <button type="button" class="flex justify-center items-center bg-emerald-500 hover:opacity-90 rounded-full w-9 h-9 font-medium text-neutral-100 text-sm transition-opacity" aria-label="Open user menu" @click="toggle">
