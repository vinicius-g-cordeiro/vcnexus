                ...
                <tr v-for="user in users" :key="user.id" index="id" class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                  ...
                  <td class="px-5 py-4">
                    <div class="flex items-center gap-3">
                      ...
                      <div class="min-w-0">
                        ...
                        <p class="flex flex-wrap gap-2 mt-0.5 text-zinc-200 text-xs">
                          <b class="bg-emerald-500 p-1 rounded-sm" v-for="rol in getRoles(user.roles)" :key="rol.value">
                            {{ rol.label }}
                          </b>
                        </p>
                  ...
                  <td class="flex justify-center px-5 py-4">
                    <div class="flex flex-wrap gap-1">
                      ...
                      <template v-if="(authStore.isSuperAdmin && user.uuid !== authStore.sessionUser?.uuid) || (!user.roles?.includes('1') && (!user.roles?.includes('2')) && user.uuid !== authStore.sessionUser?.uuid)">
...
const getRoles = (userRoles) => {
  return [...new Set(userRoles.map(Number))]
    .map(role => roles.find(item => item.value === role))
    .filter(Boolean);
};
const getRoleLabels = (userRoles) => {
  return userRoles
    .map(role => roles.find(item => Number(item.value) === Number(role)))
    .filter(Boolean)
    .map(role => role.label);
};
