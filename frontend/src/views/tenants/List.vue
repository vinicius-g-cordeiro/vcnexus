<template>
  <section class="space-y-6 mx-auto px-4 sm:px-6 lg:px-8 py-6 w-full max-w-7xl">
    ...
    <!-- Search -->
    <Fieldset :legend="t('tenants.list.search.legend')" icon="bi-people-fill" :actions="[ {url: '/tenants/new/', name: t('tenants.list.actions.new'), icon: 'bi bi-building-add'}, {url: '/tenants/reports/', name: t('tenants.list.actions.reports'), icon: 'bi bi-file-spreadsheet'},{url: '/tenants/users/', name: t('tenants.list.actions.users'), icon: 'bi bi-people'} ]">
                ...
                <tr v-for="(tenant, index) in tenants" :key="tenant.id" index="id" class="hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors">
                  ...
                  <td class="px-5 py-4 text-neutral-500">
                    <div class="flex items-center gap-3">
                      ...
                      <div class="min-w-0">
                        ...
                        <template v-if="tenant.last_login">
                          <p class="mt-0.5 text-neutral-500 text-xs">
                            <b>{{ t('tenants.list.results.headers.last_login') }}: </b>{{ formatDate(tenant.last_login,true) }}
                          </p>
                        </template>
                      </div>
                  ...
                  <td class="px-5 py-4">
                    ...
                    <div class="flex flex-wrap justify-evenly gap-1">
                      <Button variant="link" :to='"tenant/view/"+tenant.id' :title="t('tenants.list.results.actions.view')"><i class="bi bi-eye"></i></Button>

                      <Button variant="link" :to='"tenant/edit/"+tenant.id' :title="t('tenants.list.results.actions.edit')"><i class="bi bi-pencil-square"></i></Button>

                      <Button variant="link" :to='"tenant/delete/"+tenant.id' :title="t('tenants.list.results.actions.delete')"><i class="text-red-500 bi bi-trash"></i></Button>

                      <Button variant="link" :to='"tenant/block/"+tenant.id' :title="t('tenants.list.results.actions.block')"><i class="text-red-200 bi-ban bi"></i></Button>
