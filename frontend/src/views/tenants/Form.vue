    ...
    <div class="mx-auto px-4 sm:px-6 py-10 max-w-5xl">
      <div class="flex flex-col mb-8">
        <div class="flex flex-row justify-end gap-2">
          <h1 class="ms-0 me-auto font-semibold text-2xl tracking-tight">{{ isView ? 'View Tenant' : (isNew ? 'New tenant' : 'Edit Tenant') }}</h1>
          <template v-if="!isNew">
          ...
          <span class="">
            <Button variant="outline" :to="{ name: 'tenants.list' }" :title="t('tenants.list.results.actions.list')"><i class="bi bi-people-fill"></i> {{ t('tenants.list.results.legend') || 'List Tenants' }} </Button>
          </span>
        </div>
...
function mapFormToApiPayload() {
  return {
    ...
    subscription_plan: tenant.settings.subscriptionPlan,
