...
class Schema {
    ...
    #[Column(type: 'UUID', default: 'uuidv7()', nullable: false, comment: 'Unique ID for the records on this table')]
    public ?string $uuid = '';
    ...
    public static function policies(object $object) : array {
        $reflection = new ReflectionClass($object);
        $values = [];

        
        foreach($reflection->getAttributes() as $attributes){
            if($attributes->getName() !== Policy::class) continue;
            $values[] = $attributes->newInstance();
        }   
    ...
    public static function rls(object $object) : array {
        $reflection = new ReflectionClass($object);
        $values = [];

        
        foreach($reflection->getAttributes() as $attributes){
            if($attributes->getName() !== RowLevelSecurity::class) continue;
            $values[] = $attributes->newInstance();
        }   
    ...
    public static function functionAtt(object $object) : array {
        $reflection = new ReflectionClass($object);
        $values = [];

        
        foreach($reflection->getAttributes() as $attributes){
            if($attributes->getName() !== FunctionAtt::class) continue;
            $values[] = $attributes->newInstance();
        }   
